...
function handleListPartners(req, res) {
  const partners = rowsOf('SELECT * FROM partners ORDER BY name');
  const withLedger = partners.map(p => {
    const account = findAccountByName(p.name);
    return { ...p, ledger_balance: account ? accountBalance(account.id) : null };
  });
  json(res, 200, { partners: withLedger });
}
...
// Party Ledger (Phase 3): a partner's transaction history is whatever
// ledger entries reference an account with the same name. We match by
// name rather than a stored foreign key, so it keeps working even for
// partners that existed before the ledger did, or accounts created
// directly through a ledger entry before the partner record existed.
function handlePartnerLedger(req, res, id) {
  const partner = oneOf('SELECT * FROM partners WHERE id = ?', [id]);
  if (!partner) return notFound(res);
  const account = findAccountByName(partner.name);
  if (!account) {
    return json(res, 200, { partner, account: null, balance: null, entries: [] });
  }
  const entries = rowsOf(
    `SELECT l.*, c.name AS credit_account, d.name AS debit_account
     FROM ledger_entries l JOIN accounts c ON c.id=l.credit_account_id JOIN accounts d ON d.id=l.debit_account_id
     WHERE l.credit_account_id = ? OR l.debit_account_id = ?
     ORDER BY l.date DESC, l.id DESC`,
    [account.id, account.id]
  );
  json(res, 200, { partner, account, balance: accountBalance(account.id), entries });
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if ((m = pathname.match(/^\/api\/partners\/(\d+)\/ledger$/)) && req.method === 'GET') return handlePartnerLedger(req, res, m[1]);
