...
async function handleCreatePurchase(req, res, user) {
  ...
  const info = run(
    `INSERT INTO purchases (date, crop, party_name, qty, rate, total, transport, status, created_by)
     VALUES (?,?,?,?,?,?,?,?,?)`,
    [body.date, body.crop.trim(), body.party_name.trim(), qty, rate, total, transport, computeStatus(body.status), user.id]
  );
...
async function handleUpdatePurchase(req, res, id) {
  ...
  const total = qty * rate;
  run(`UPDATE purchases SET date=?, crop=?, party_name=?, qty=?, rate=?, total=?, transport=?, status=? WHERE id=?`,
    [merged.date, merged.crop.trim(), merged.party_name.trim(), qty, rate, total, transport, computeStatus(merged.status), id]);
...
async function handleCreateSale(req, res, user) {
  ...
  const info = run(
    `INSERT INTO sales (date, crop, buyer_name, qty, rate, total, status, created_by)
     VALUES (?,?,?,?,?,?,?,?)`,
    [body.date, body.crop.trim(), body.buyer_name.trim(), qty, rate, total, computeStatus(body.status), user.id]
  );
...
async function handleUpdateSale(req, res, id) {
  ...
  const total = qty * rate;
  run(`UPDATE sales SET date=?, crop=?, buyer_name=?, qty=?, rate=?, total=?, status=? WHERE id=?`,
    [merged.date, merged.crop.trim(), merged.buyer_name.trim(), qty, rate, total, computeStatus(merged.status), id]);
...
function handleStock(req, res) {
  ...
  const stock = crops.map(c => {
    ...
    return { crop: c.name, purchased: p.q, sold: s.q, inStock: Math.round((p.q - s.q) * 100) / 100 };
  });
  json(res, 200, { stock });
}
...
function handleExport(req, res, query) {
  ...
  if (type === 'purchases') {
    headers = ['Date', 'Crop', 'Party', 'Qty (Q)', 'Rate/Q', 'Total', 'Transport', 'Status'];
    rows = rowsOf('SELECT * FROM purchases ORDER BY date DESC').map(p => [p.date, p.crop, p.party_name, p.qty, p.rate, p.total, p.transport, p.status]);
    filename = 'purchases';
  } else if (type === 'sales') {
    headers = ['Date', 'Crop', 'Buyer', 'Qty (Q)', 'Rate/Q', 'Total', 'Status'];
    rows = rowsOf('SELECT * FROM sales ORDER BY date DESC').map(s => [s.date, s.crop, s.buyer_name, s.qty, s.rate, s.total, s.status]);
    filename = 'sales';
...
async function handleImport(req, res, user, query) {
  ...
  const insertPurchase = db.prepare(`INSERT INTO purchases (date, crop, party_name, qty, rate, total, transport, status, created_by) VALUES (?,?,?,?,?,?,?,?,?)`);
  const insertSale = db.prepare(`INSERT INTO sales (date, crop, buyer_name, qty, rate, total, status, created_by) VALUES (?,?,?,?,?,?,?,?)`);
  ...
  dataRows.forEach((row, i) => {
    try {
      if (importType === 'purchases') {
        ...
        insertPurchase.run(date, crop, party, qty, rate, qty * rate, transport, status, user.id);
        ...
        insertSale.run(date, crop, buyer, qty, rate, qty * rate, status, user.id);
      } else {
        const date = String(row[idx['date']] ?? '').trim();
...
// ================================================================
// AI (server-side proxy — key never sent to the browser)
// ================================================================
async function callAnthropic(messages, maxTokens, system) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { error: 'AI features are not configured on this server (missing ANTHROPIC_API_KEY).' };
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: maxTokens || 500, system, messages })
    });
...
const SYSTEM_PROMPT = 'You are the AI assistant for Aditya Trading Company, a grain mandi trading firm in Madhya Pradesh, India. Be friendly, professional, concise. Mix English with Hindi mandi terms. Under 120 words.';

async function handleAiChat(req, res) {
  let body; try { body = await readJson(req); } catch (e) { return badRequest(res, e.message); }
  if (!Array.isArray(body.messages) || !body.messages.length) return badRequest(res, 'messages array is required');
  const result = await callAnthropic(body.messages, 300, SYSTEM_PROMPT);
...
async function handleAiInsights(req, res) {
  const dash = oneOf('SELECT 1');
  const rev = oneOf('SELECT COALESCE(SUM(total),0) AS v FROM sales').v;
  const exp = oneOf('SELECT COALESCE(SUM(total),0) AS v FROM expenses').v;
  const purchaseCost = oneOf('SELECT COALESCE(SUM(total+transport),0) AS v FROM purchases').v;
  const stockRows = rowsOf('SELECT name FROM crops').map(c => {
    const p = oneOf('SELECT COALESCE(SUM(qty),0) AS q FROM purchases WHERE crop=?', [c.name]).q;
    const s = oneOf('SELECT COALESCE(SUM(qty),0) AS q FROM sales WHERE crop=?', [c.name]).q;
    return `${c.name}: ${p - s}Q in stock`;
  }).join(', ');

  const prompt = `Current business snapshot — Revenue: Rs${rev}, Purchase cost: Rs${purchaseCost}, Expenses: Rs${exp}, Stock: ${stockRows}. Give 3 short actionable insights for the trading business owner.`;
  const result = await callAnthropic([{ role: 'user', content: prompt }], 400, SYSTEM_PROMPT);
  if (result.error) return json(res, 502, { error: result.error });
  json(res, 200, { text: result.text });
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/api/ai/chat' && req.method === 'POST') return handleAiChat(req, res);
    if (pathname === '/api/ai/insights' && req.method === 'GET') return handleAiInsights(req, res);
